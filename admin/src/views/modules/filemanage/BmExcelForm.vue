  ...
  <el-dialog
    ...
    <el-form :model="inputForm" ref="inputForm" v-loading="loading" :class="method==='view'?'readonly':''"  :disabled="method==='view'"
      ...
      <el-row  :gutter="15">
        ...
        <el-col :span="24">
            <el-form-item label="内容" prop="excelCentent"
                :rules="[
                  {required: true, message:'excel内容不能为空', trigger:'blur'}
                 ]">
              <el-upload ref="excelCentent"
                    v-if="visible"
                    :action="`${this.$http.BASE_URL}/sys/file/webupload/upload?uploadPath=/filemanage/bmExcel`"
                    ...
                    <div slot="tip" class="el-upload__tip">添加excel文件</div>
                  </el-upload>
           </el-form-item>
        </el-col>
        </el-row>
      ...
      init (method, id) {
        ...
        this.$nextTick(() => {
          ...
          if (method === 'edit' || method === 'view') { // 修改或者查看
            this.loading = true
            this.$http({
              url: `/filemanage/bmExcel/queryById?id=${this.inputForm.id}`,
              method: 'get'
            }).then(({data}) => {
